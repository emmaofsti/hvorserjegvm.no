...
export default function HomeClient({ venues }: { venues: Venue[] }) {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6">
      ...
      <section className="mb-4 h-[55vh] min-h-[420px] overflow-hidden rounded-xl border border-[var(--border)]">
        <VenuesMap venues={filtered} userLocation={userLocation} highlightId={highlightId} />
      </section>

      <section className="mb-4 flex flex-wrap items-center gap-4 rounded-xl border border-[var(--border)] bg-[var(--bg-surface)] px-4 py-2.5 text-xs">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          {locError && <span className="text-amber-400">{locError}</span>}
        </div>
      </section>

      <section className="mb-6 rounded-xl border border-[var(--border)] bg-[var(--bg-surface)] p-4">
        <div className="mb-5">
          <label className="relative block">
            ...
            <Input
              ...
              placeholder={`Søk blant ${venues.length} steder — navn, nabolag, adresse…`}
            ...
            {search && (
              <button
                type="button"
                onClick={() => setSearch("")}
                className="absolute right-2 top-1/2 -translate-y-1/2 rounded p-1 text-slate-400 hover:bg-[var(--border)] hover:text-slate-100"
                aria-label="Tøm søk"
              >
                ✕
              </button>
      ...
      <section className="mb-3 flex items-end justify-between gap-3">
        <div>
          ...
          <h2 className="text-lg font-semibold text-slate-100">{filtered.length} steder</h2>
        </div>
        <button
          onClick={() => {
